...
test('desktop submits only explicitly, uses IPC for images, and withdraws public/private feedback',async()=>{
 ...
 try{
  ...
  await page.locator('input[type=file]').setInputFiles(Array.from({length:6},(_,i)=>({name:'acceptance-'+i+'.png',mimeType:'image/png',buffer:Buffer.from(png,'base64')})));await expect(page.locator('.fb-image-preview')).toBeVisible();await page.locator('.fb-submit').click();await expect(page.locator('.fb-card')).toHaveCount(1);await expect(page.locator('.fb-card .fb-thumbnail')).toHaveCount(5);await page.locator('.fb-card .fb-thumbnail').first().scrollIntoViewIfNeeded();await expect(page.locator('.fb-card .fb-thumbnail img').first()).toHaveJSProperty('naturalWidth',80);await page.getByRole('button',{name:'Next images',exact:true}).click();await expect(page.locator('.fb-gallery-controls').first()).toContainText('2–6 / 6');await page.locator('.fb-thumbnail').last().click();await expect(page.locator('.fb-lightbox')).toBeVisible();await expect(page.locator('.fb-lightbox img')).toHaveJSProperty('naturalWidth',80);await page.getByRole('button',{name:'Close',exact:true}).click();
